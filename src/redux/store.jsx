...
const cartSlice = createSlice({
    ...
    reducers: {
        ...
        //     const findTopping = getAllTopping().find(item => item.id === existingProduct.topping);
        //     if (existingProduct) {
        //         existingProduct.quantity += 1
        //         existingProduct.total = existingProduct.quantity * (existingProduct.price + findTopping.price)
        //     } else {
        ...
        addToCart: (state, action) => {
            ...
            const findTopping = getAllTopping().find(item => topping.includes(item.id));
            ...
            const existingProduct = state.products.find(
                item => item.id === id && item.topping === toppingName && item.sweet === sweetName
            );
        
            if (existingProduct) {
                existingProduct.quantity += 1;
                existingProduct.total = existingProduct.quantity * (price + toppingPrice);
            } else {
                state.products.push({
                    ...action.payload,
                    sweet: sweetName,
